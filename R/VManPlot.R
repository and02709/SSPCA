...
#' @export
#' @examples VManPlot(Zlist, sepAnalysis, nresp, man.thresh, resp.names, 
#'   specific.groups=NULL, exclude.groups=NULL, special.colors=NULL)


VManPlot <- function(Zlist, sepAnalysis, nresp, man.thresh, resp.names, 
                       specific.groups=NULL, exclude.groups=NULL, special.colors=NULL){
  if(sepAnalysis){
    hold.pattern <- Zlist
    ...
    if(n.resp==1){
      ...
      for(j in 1:num.vec){
        ...
        fname <- paste0("/outputfolder/",resp.names, " Volume Cifti Eigenvector ", j, ".png")
        ...
        gg_plot_title <- paste0(resp.names, " Volume Cifti Eigenvector ", j)
      ...
      for(i in 1:n.resp){
        ...
        for(j in 1:num.vec){
          ...
          #wd <- getwd()
          #fname <- file.path(wd, "outputfolder", name_img)
          fname <- paste0("/outputfolder/",resp.names[i], " Volume Cifti Eigenvector ", j, ".png")
          if(i==1 && j==1){
            plot.list <- list(fname)
          }
          else{
          ...
          gg_plot_title <- paste0(resp.names[i], " Volume Cifti Eigenvector ", j)
    ...
    for(j in 1:num.vec){
      ...
      #wd <- getwd()
      #fname <- file.path(wd, "outputfolder", name_img)
      fname <- paste0("/outputfolder/","Volume Cifti Eigenvector ", j, ".png")
      ...
      gg_plot_title <- paste0("Volume Cifti Eigenvector ", j)
  ...
  Uabs <- apply(Umat,2,abs())
  dftemp <- read.table(file=fpath, header=T)
  volume.data.frame <- data.frame(dftemp, Uabs)
  colnames(volume.data.frame) <- c("loading index", "domain", paste0("u",1:num.vec))
  
  return(list(plot.list=plot.list, volume.data.frame))
